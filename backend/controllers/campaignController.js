...
exports.getNearbyCampaigns = async (req, res) => {
  try {
    ...
    const campaigns = await Campaign.find(query).populate('ngoId', 'name').lean();
...
exports.getMapCampaigns = async (req, res) => {
  try {
    const { longitude, latitude, category, maxDistance = 100000 } = req.query;
...
exports.getTodayCampaigns = async (req, res) => {
  try {
    const { longitude, latitude, category, maxDistance = 50000 } = req.query;
...
exports.getCampaignDetails = async (req, res) => {
  try {
    ...
    if (now >= start && now <= end) liveStatus = 'approved';
